...
export const chooseNodeMongoApiDBServer = async (templateName, pathToCheck) => {
  ...
  // const defaultConnectionType = 'atlas';
  // const connectionType = [defaultConnectionType, 'local'];

  const [SwitchToAtlas, SwitchToLocal, IgnorePrompt, InstallAtlasConnection, InstallLocalConnection] = ['Switch to atlas connection', 'Switch to local connection', 'Ignore prompt', 'Install Atlas Connection Files', 'Install Local Connection Files'];
  const actions = {
    atlas: [SwitchToLocal, IgnorePrompt],
    local: [SwitchToAtlas, IgnorePrompt],
    installNew: [InstallAtlasConnection, InstallLocalConnection],
  };
  ...
  const questionPush = () => {
    ...
    if (AtlasSetOfConnectionFiles) {
      connectionQuestions.push({
        ...
        choices: actions.atlas,
        default: actions.atlas[0],
      });
    }

    if (LocalSetOfConnectionFiles) {
      connectionQuestions.push({
        ...inquiryType,
        message: 'Your nodejs API already uses the LOCAL server and db connection type.\n  Which of these actions would you like to take?',
        choices: actions.local,
        default: actions.local[0],
      });
    }

    if (AllDbServerFileNames && (!AtlasSetOfConnectionFiles || !LocalSetOfConnectionFiles)) {
      connectionQuestions.push({
        ...inquiryType,
        message: 'Your nodejs API cannot run without having a complete set of (db and server) connection files. ',
        choices: actions.local,
        default: actions.local[0],
      });
    }
